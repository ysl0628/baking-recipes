...
## 🔒 給任何數值前的兩道強制檢查（2026-09 連兩爐失敗換來的）

> **失敗的根本不是技術，是「換了一個變因，卻沿用舊變因調出來的數字」。**

### ① 先翻使用者自己的紀錄，再給數字

**使用者早就試出來的值 > 你從文獻推的值 > 你的印象。**
給水合、發酵時數、酵母量、溫度這類數字前，**先查 `logs/`、各 reference 的【實測記錄】區、
以及使用者的 Notion 食譜庫**（§5）有沒有已驗證過的數字。

> 🚨 **實例**：使用者 2026-08 已用實驗解出「這套設備＋手法的可操作水合是 **69%**」，
> 9 月換粉時 AI 與使用者都沒去翻，直接沿用 70% → 連兩爐攤成麵糊無法整形。
> **答案八月就在檔案裡。**

### ② 換變因時，數字要「換算」不能「沿用」

**任何已驗證的數字都綁在當時的條件上。** 條件一變，數字必須重算：

| 換掉什麼 | 要換算什麼 |
|---|---|
| **麵粉** | **有效水合**（法國粉 −5pp、高吸水粉 +、全穀 +）→ `flour-notes.md` |
| **拿掉全穀／配料** | 吸水大戶消失 → **水合要跟著降**，不是維持 |
| **環境溫度** | 發酵時數、酵母量、水溫（`ddt.py`）、麵粉容錯需求 |
| **麵種類型** | 判準整組換（酸種 vs 商業酵母，見 `fermentation-guide.md` §1） |

⚠️ **最危險的是「帳面上看起來更保守」的情況**：
拿掉 31% 全麥、水合從 72% 降到 70%——看起來降了 2%，**有效水合其實升了 5~8%**。

### ③ 推薦麵粉時必問操作環境溫度

不能只看「蛋白質（結構）× 灰分（風味）」兩個維度。**第三個維度是容錯度**：
石磨／高灰分粉風味最好，但**酵素活性最高、容錯最低**，高溫環境或練習階段不適合
（詳見 `troubleshooting.md` §五）。
